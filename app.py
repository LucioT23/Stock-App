...
def cleaning_data(df):
  ...
  df["Code groupe DISE"] = df["Code groupe DISE"].str.replace(r'[,-/]', ';').str.replace(r';\s+', ';')
...
def client_MWM(df_mwm, df_Planning_data, plus_recente, portail):
  ...
  if portail == 'EWOCS':
    ...
    Code_Opache = df_mwm_filtered['Code groupe DISE'].loc[df_mwm_filtered['title'] == 'Opache 5']
...
if choice == "Download":
    ...
    if file: 
        ...
        for i, val in enumerate(df[col]):
          try:
              df[col][i] = pd.to_datetime(val, format='%Y %m %d')
          except ValueError:
              df[col][i] = pd.NaT
